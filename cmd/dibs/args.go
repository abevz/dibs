...
var commandRoutes = map[string]argRoute{
	...
	"hooks complete":      {"--pr-url --commit-sha --branch --note", 0},
	"daemon start":        {"", 0}, "daemon stop": {"", 0},
	"project add": {"--key --name --description", 0}, "project list": {"", 0},
	"repo add": {"--project --logical-name --canonical-git-dir --default-branch --remotes", 0}, "repo list": {"--project", 0}, "repo relocate": {"--repo --new-path --operation-id", 0},
	...
	"issue run":              {"--actor --ttl --close-resolution --branch --pr-url --commit-sha --note --invocation-mode --require-complete? --publish?", 1},
	...
	"issue close":            {"--resolution --expected-version --lease-token --lease-generation --branch --pr-url --commit-sha --note --invocation-mode --operation-id --publish?", 1},
