...
| R-09 idempotent mutations | `afc-111`, `afc-112`, `afc-113` | Idempotency; Race 6 | `afc-110` proves a replayed committed close is non-duplicating but returns `version_conflict`; original-outcome replay remains planned in `afc-111` through `afc-113` |
