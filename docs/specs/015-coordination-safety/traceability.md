...
| R-09 idempotent mutations | `afc-111`, `afc-112`, `afc-113` | Idempotency; Race 6 | ledger and claim replay implemented by `afc-111`; `afc-112` locally verifies create replay, exact original short ID, one issue/event/sequence increment, mismatched-payload conflict, independent-connection races, and restart durability (owner review pending); lifecycle-mutation retry remains `afc-113` |
