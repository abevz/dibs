...
| R-09 idempotent mutations | `afc-111`, `afc-112`, `afc-113` | Idempotency; Race 6 | ledger and claim replay implemented by `afc-111`; create replay merged in `afc-112` PR #72; `afc-113` tests heartbeat expiry, release, update version/outcome, handoff note ID, close metadata, cross-kind conflicts, and independent-connection heartbeat races (owner review pending) |
