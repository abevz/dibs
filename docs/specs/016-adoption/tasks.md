...
| Recover tag publication and public smoke | `afc-153` | A, release follow-up | `afc-128` | Restore and verify the remote annotated tag ref after checkout; a regression fixture reproduces checkout's peeled-tag overwrite. Dispatch public-URL installation and first-use smoke on all four advertised runners for an already published version without republishing it. Record RC2 failure/recovery and successful public-smoke evidence. |
...
| Manual claim process diagnostics | `afc-152` | A, watch follow-up | `afc-151` | New CLI manual claims report an available caller ancestor PID and host in watch text/JSON; explicit session IDs, old/MCP claims, and retry replay behave as documented; diagnostics never imply liveness or ownership. |
...
| Swarm launcher | `afc-133` | B | `afc-102`, `afc-120`–`afc-122`, packet approval | `dibs swarm -n 3 -- <cmd>` uses distinct claims/worktrees and branches; built-in Claude preset; worker failure isolated; failed/incomplete worktrees kept and explicit cleanup removes only verified merged unreferenced ones; no default PR; short recipes for candidate harnesses, at least three verified end to end; recorded demo. Independent extension; does not gate `afc-134`. |
| README race demo | `afc-158` | C | `afc-130`, `afc-132` | Delivered in PR #103: `contrib/demo/record.sh` records two agents contesting one claim with exactly one owner, distinct completed issues, and `HANDOFF` recovery, using isolated temporary state. |
| Launch | `afc-134` | C | `afc-130`, `afc-132`, `afc-158`; public release and owner sign-off | The `afc-158` demo, re-run against the published release binaries, shows distinct issues with no double claim; owner writes/publishes final post with accurate single-writer and shared-state claims, without a Beads comparison. Swarm (`afc-133`) is not a prerequisite. |
